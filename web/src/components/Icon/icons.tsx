...
export default function Icon({ kind, href, size = 8 }: IconProps) {
  const IconSvg = iconComponents[kind];
  const iconStyle = `fill-current text-gray-700 hover:text-primary-500 h-${size} w-${size}`;
  const ancorStyle = `text-sm text-gray-500 transition hover:text-gray-600 h-6 w-6`;

  return (
    <a
      className={ancorStyle}
      target="_blank"
      rel="noopener noreferrer"
      href={href}
    >
      <span className="sr-only">{kind}</span>
      <IconSvg className={iconStyle} />
    </a>
  );
}
